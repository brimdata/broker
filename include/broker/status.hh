...
public:
  ...
  [[deprecated("please use the template versions instead")]] static status
  make(sc code, endpoint_info ei, std::string msg) {
    BROKER_ASSERT(code == sc::peer_added || code == sc::peer_removed
                  || code == sc::peer_lost);
    return {code, std::move(ei), std::move(msg)};
  }
