...
TEST(the connector asks the middleman for actor handles) {
  set_mm(0);
  self->send(aut, atom::peer::value, network_info{"localhost", 8080});
  expect((atom_value, network_info), from(self).to(aut));
  expect((atom_value, std::string, uint16_t), from(aut).to(mm));
