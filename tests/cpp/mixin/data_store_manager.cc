...
struct fixture : test_coordinator_fixture<> {
  ...
  fixture() : clock(&sys, true) {
    ...
    anon_send(peers["A"], atom::peer::value, peer_id{"B"}, peers["B"]);
    run();
...
TEST(peers propagate new masters) {
  auto res = self->request(peers["A"], caf::infinite, atom::store::value,
                           atom::master::value, atom::attach::value, "kono",
                           memory, backend_options({{"foo", 4.2}}));
  run();
  ...
  inject((atom::store, atom::master, atom::attach, std::string, backend,
         ...
         from(self)
           .to(peers["A"])
           .with(atom::store::value, atom::master::value, atom::attach::value,
                 "kono", memory, backend_options({{"foo", 4.2}})));
  ...
  inject((atom::store, atom::master, atom::get, std::string),
         from(self)
           .to(peers["A"])
           .with(atom::store::value, atom::master::value, atom::get::value,
                 "kono"));
  expect((caf::actor), from(peers["A"]).to(self).with(kono_master));
...
TEST(clones wait for remote masters to appear) {
  auto res = self->request(peers["B"], caf::infinite, atom::store::value,
                           atom::clone::value, atom::attach::value, "kono", 1.0,
                           1.0, 1.0);
  consume_messages();
  caf::actor clone;
  res.receive(
  ...
  caf::anon_send(peers["A"], atom::store::value, atom::master::value,
                 atom::attach::value, "kono", memory,
